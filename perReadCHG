...
void *perReadCHGMetrics(void *foo) {
    ...
    if(!os) {
        fprintf(stderr, "Couldn't allocate space the for kstring_t structure in perReadCHG!\n");
        return NULL;
    }
...
void perReadCHG_usage() {
    fprintf(stderr, "\nUsage: MethylDackel perReadCHG [OPTIONS] <ref.fa> <input>\n");
    fprintf(stderr,
"\n"
"This program will compute the average CHG methylation level of a given read.\n"
...
int perReadCHG_main(int argc, char *argv[]) {
    Config config;
    FILE *ofile = stdout;
    int i, keepStrand = 0;
    faidx_t *fai;
    ...
    //Defaults
    config.keepCpG = 0; config.keepCHG = 1; config.keepCHH = 0;
    ...
    for(i=0; i < config.nThreads; i++) pthread_create(threads+i, NULL, &perReadCHGMetrics, &config);
